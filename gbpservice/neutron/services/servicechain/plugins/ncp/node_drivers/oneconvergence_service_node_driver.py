...
class OneConvergenceServiceNodeDriver(heat_node_driver.HeatNodeDriver):
    ...
    def get_plumbing_info(self, context):
        ...
        request_plumber_info = {'management': [], 'provider': [{}],
                                'consumer': []}
        if service_type in [pconst.FIREWALL, pconst.VPN]:
            ...
            if (self._check_for_fw_vpn_sharing(context, service_type) and
                ...
                return False
            request_plumber_info['consumer'] = [{}]
            request_plumber_info['plumbing_type'] = 'gateway'
        else:  # Loadbalancer
            request_plumber_info['plumbing_type'] = 'endpoint'

        return request_plumber_info
    ...
    def create(self, context):
        ...
        mgmt_fip = self._instantiate_servicevm(context)
        stack_template, stack_params = self._update_node_config(
            context, mgmt_fip=mgmt_fip)
    ...
    def delete(self, context):
        ...
        ports_to_cleanup = ports_to_cleanup or {}
        if ports_to_cleanup.get('mgmt_port_id'):
            ...
            for policy_target in policy_targets:
                ...
                try:
                    context.gbp_plugin.delete_policy_target(
                        context.plugin_context, policy_target['id'],
                        notify_sc=False)
                except Exception:
    ...
    def _update(self, context, pt_added_or_removed=False):
        ...
        mgmt_fip = service_info['floating_ip']
        stack_template, stack_params = self._update_node_config(
            context, update=True, mgmt_fip=mgmt_fip)
    ...
    def _create_service_management_port(self, context):
    ...
    def _get_service_targets(self, context):
        provider_port = consumer_port = None
        ...
        for service_target in service_targets:
            ...
            if service_target.relationship == 'provider':
                provider_port = port
            if service_target.relationship == 'consumer':
                consumer_port = port
        return (provider_port, consumer_port)

    def _get_shared_service_targets(self, context, service_type):
    ...
    def _update_firewall_template(self, context, stack_template):
        ...
        for subnet in provider_l2p_subnets:
            if not subnet['name'].startswith(apic_mapping.APIC_OWNED_RES):
    ...
    # REVISIT(Magesh): Remove this when the plumber supports FIP for VPN
    def _allocate_floating_ip(self, context, fixed_port_id, l3p):
        external_segments = l3p.get('external_segments').keys()
        if not external_segments:
            LOG.error(_("Allocating Floating IP not possible because "
                        "l3policy does not have an attached external segment"))
            raise L3PolicyNotAssociatedWithES()
        external_segment = context.gbp_plugin.get_external_segment(
            context.plugin_context, external_segments[0])
        ext_sub = context.core_plugin.get_subnet(
            context.plugin_context, external_segment['subnet_id'])
        ext_net_id = ext_sub['network_id']
        try:
            LOG.error(_("Tenant id: %s") %(context.plugin_context.tenant_id))
            attrs = {'tenant_id': context.plugin_context.tenant_id,
                     'floating_network_id': ext_net_id}
            if fixed_port_id:
                attrs.update({"port_id": fixed_port_id})
            floatingip_create_data = {'floatingip': attrs}
            return context.l3_plugin.create_floatingip(
                context.plugin_context, floatingip_create_data)
        except Exception:
    ...
    def _instantiate_servicevm(self, context):
        ...
        if service_info:
            svc_mgmt_port = floatingip_id = None
            mgmt_floating_ip = service_info['floating_ip']
        else:
            svc_mgmt_port, floatingip = (
                self._create_service_management_port(context))
            floatingip_id = floatingip['id']
            mgmt_floating_ip = floatingip['floating_ip_address']

        provider_port, consumer_port = self._get_service_targets(context)
        if service_type == pconst.LOADBALANCER:
            ...
            provider_port['allowed_address_pairs'] = [{'ip_address': vip_ip}]
            port = {}
            port['port'] = provider_port
            context.core_plugin.update_port(
                context.plugin_context, provider_port['id'], port)

        # TODO(Magesh): Change the arguments to a dictionary
        self.svc_mgr.create_service_instance(
            context=context.plugin_context,
            tenant_id=context.provider['tenant_id'],
            service_chain_instance_id=sc_instance['id'],
            service_type=service_type,
            provider_network_port=provider_port['id'],
            consumer_network_port=None,
            stitching_network_port=(consumer_port['id'] if consumer_port
                                   else None),
    ...
    def _update_node_config(self, context, update=False, mgmt_fip=None):
        ...
        if service_type == pconst.LOADBALANCER:
            ...
            config_param_values['service_chain_metadata'] = (
                SC_METADATA % (context.instance['id'], mgmt_fip,
                               provider_port_mac))
            ...
            firewall_desc = {'vm_management_ip': mgmt_fip,
            ...
            desc = ('fip=' + mgmt_fip + ";tunnel_local_cidr=" +
