...
from f5_constants import F5_CONF_FILE
...
class BigIPLicenseServerCommon(object):
    def __init__(self, regkey):
        if is_production_key(regkey):
            self.license_server = 'activate.f5.com'
        else:
            self.license_server = 'authem.f5net.com'

        confparser = ConfigParser.ConfigParser()
        confparser.read(F5_CONF_FILE)
        try:
            is_proxy_enabled = confparser.get('F5_CONFIG', 'proxy_server')
            if is_proxy_enabled == 'True':
                self.proxy = {}
                self.proxy['http'] = confparser.get('F5_CONFIG', 'http_proxy')
                self.proxy['https'] = confparser.get('F5_CONFIG', 'https_proxy')
            else:
                self.proxy = None
        except (ConfigParser.NoOptionError, ConfigParser.NoSectionError) as e:
            self.proxy = None
            
    def get_license_server_url(self, wsdl=None):
    ...
    def test_license_server(self, wsdl=None):
        url = self.get_license_server_url(wsdl)
        try:
            if self.proxy:
                client = suds.client.Client(url=url, location=url, proxy=self.proxy)
            else:
...
class BigIpLicenseCommon(BigIPLicenseServerCommon):
    def __init__(self, regkey, username, password, hostname):
        ...
        super(BigIpLicenseCommon, self).__init__(regkey)
...
def main():
    ...
    try:
        ...
        if state == "present":
            ...
            lic_server = obj.test_license_server(wsdl)
