""" Module for managing F5 BIG-IP device interfaces in Neutron """
...
class VLANBindingBase(object):
    """ Base Class for device interface to port bindings """
    def __init__(self, conf, driver):
        ...
        LOG.debug(_('reading static device interface port bindings'))
        if self.conf.interface_port_static_mappings:
            ...
            LOG.debug(_('interface_port_static_mappings not configured'))
                    ...
                    for port in ports:
                        for interface in bigip.device_interfaces:
                            if not interface == 'mgmt':
                                ...
                                LOG.debug(_('adding mapping information device'
                                            '%s interface %s to port: %s'
                                            % (bigip.device_name,
                                               interface,
                                               port['id'])))
            self.__initialized__bigip_ports = True
            LOG.debug(_('interface bindings after initialization are: %s'
                        % self.interface_binding_mappings))
            for bigip in self.driver.get_all_bigips():
...
class NullBinding(VLANBindingBase):
    ...
    def allow_vlan(self, device_name=None, interface=None, vlanid=0):
        ...
        if vlanid == 0:
            return
        LOG.debug(_('checking for port bindings '
