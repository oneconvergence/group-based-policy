...
class FwGenericConfigDriver(base_driver.BaseDriver):
    ...
    def _configure_static_ips(self, resource_data):
        ...
        msg = (log_meta_data + "Static IPs successfully added.")
        LOG.info(msg)
    ...
    def configure_interfaces(self, context, resource_data):
        ...
        try:
            ...
            msg = (log_meta_data + "Failed to add static IPs. Error: %s" % err)
            LOG.error(msg)
            return msg
        else:
        ...
        msg = (log_meta_data + "Initiating POST request to add persistent rule"
               " to primary service at: %r" % mgmt_ip)
        LOG.info(msg)
        try:
            resp = requests.post(url, data, timeout=self.timeout)
        except requests.exceptions.ConnectionError as err:
        ...
        msg = (log_meta_data + "Persistent rule successfully added.")
        LOG.info(msg)
        return const.STATUS_SUCCESS
    ...
    def _clear_static_ips(self, resource_data):
        ...
        msg = (log_meta_data + "Initiating POST request to remove static IPs"
               "for primary service at: %r" % mgmt_ip)
        LOG.info(msg)
        try:
            ...
            msg = (log_meta_data + "Unexpected ERROR happened  while removing "
        ...
        try:
            ...
            return msg
        if not result['status']:
            msg = (log_meta_data + "Error removing static IPs. URL: %r."
                   " Reason: %s." % (url, result['reason']))
            LOG.error(msg)
            return msg

        msg = (log_meta_data + "Static IPs successfully removed.")
        LOG.info(msg)
        return const.STATUS_SUCCESS
    ...
    def clear_interfaces(self, context, resource_data):
        ...
        msg = (log_meta_data + "Initiating DELETE persistent rule.")
        LOG.info(msg)
        url = const.request_url % (mgmt_ip,
                                   self.port,
                                   'delete_rule')
        ...
        if not result['status'] or resp.status_code not in [200, 201, 202]:
            ...
            LOG.error(msg)
            raise Exception(msg)
        msg = (log_meta_data + "Persistent rule successfully deleted.")
        LOG.info(msg)
        return const.STATUS_SUCCESS
    ...
    def configure_routes(self, context, resource_data):
        ...
        try:
            ...
            msg = (log_meta_data + "Failed to establish connection to service"
                   " at: %r. ERROR: %r" % (mgmt_ip, str(err).capitalize()))
            LOG.error(msg)
            return msg
        except requests.exceptions.RequestException as err:
    ...
    def clear_routes(self, context, resource_data):
        ...
        try:
            ...
            msg = (log_meta_data + "Failed to establish connection to primary"
                   "service at: %r. ERROR: %r" % (mgmt_ip, err))
            LOG.error(msg)
            return msg
        except requests.exceptions.RequestException as err:
            msg = (log_meta_data + "Unexpected ERROR happened  while deleting "
