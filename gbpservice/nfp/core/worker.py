...
class NfpWorker(Service):
    ...
    def _process_event(self, event):
        ...
        if event.desc.type == nfp_event.SCHEDULE_EVENT:
            ...
            eh, _ = self.event_handlers.get_event_handler(event.id, module=event.desc.target)
    ...
    def _handle_poll_event(self, event):
        ...
        poll_handler = self.event_handlers.get_poll_handler(event.id, module=module)
        event_handler, _ = self.event_handlers.get_event_handler(event.id, module=module)
        try:
