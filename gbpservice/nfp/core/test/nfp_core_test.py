...
class EventsHandler(nfp_api.NfpEventHandler):
    ...
    def handle_event(self, event):
        LOG(LOGGER, 'INFO', "Handle Event %s" % (event.identify()))
        if event.id == 'EVENT_ID_1':
            # LOG(LOGGER, 'INFO', "ADDING POLL EVENT %s CURR TIME %s" %
    ...
    def handle_poll_event(self, event):
        LOG(LOGGER, 'INFO', "Event TIMEDOUT %s CURR TIME : %s" %
            (event.identify(), time.time()))

    def event_cancelled(self, event, reason):
        LOG(LOGGER, 'INFO', "Event %s cancelled reason %s" %
