...
class NfpResourceManager(NfpProcessManager, NfpEventManager):
    ...
    def _scheduled_new_event(self, event):
        ...
        if not event.sequence:
            ...
            LOG(LOGGER, 'DEBUG', "(event - %s) - sequencing" %
                (event.identify()))
    ...
    def _scheduled_event_ack(self, ack_event):
        try:
            ...
            kerr = kerr
            LOG(LOGGER, 'ERROR', "(event - %s) - acked,"
    ...
    def _scheduled_event_complete(self, event, expired=False):
        ...
        try:
            ...
            kerr = kerr
            LOG(LOGGER, 'ERROR', "(event - %s) - completed, not in cache" %
                (event.identify()))
        except AssertionError as aerr:
            aerr = aerr
            # No event manager for the event, worker could have got
            # killed, ignore.
            LOG(LOGGER, 'ERROR', "(event - %s) - assertion error" %
                (event.identify()))
            pass
        finally:
    ...
    def process_events_by_ids(self, event_ids):
        for event_id in event_ids:
            try:
                ...
                kerr = kerr
                LOG(LOGGER, 'ERROR', "%s - event missing in cache" %
                    (event_id))
    ...
    def _event_timedout(self, event):
        ...
        try:
            ...
            err = err
            LOG(LOGGER, 'ERROR', "(event - %s) - expired, not in cache" %
                (event.identify()))
        except AssertionError as aerr:
            aerr = aerr
