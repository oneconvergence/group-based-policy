...
class ConfigSession(Session):
    ...
    This is a singleton subclass of Session class which ensures that one and
    one config session only is opened.
    ...
    def setup_config_session(self):
        ...
        env['VYATTA_CONFIG_TMP'] = '/opt/vyatta/config/tmp/tmp_{0}'.format(
            identifier)
        # Add vyos session environment to system environment.
        # This is not good but actually it seems that is the only way to
        # handle a persistant vyos session after spawning a shell.
        os.environ.update(env)
        logger.info('Setting up a configuration session for Vyos')
        # Spawn shell and setup vyos config session
        if _run('{0} setupSession'.format(VYOS_SHELL_API)):
    ...
    def session_exists(self):
        ...
        """
        return False if _run('{0} inSession'.format(VYOS_SHELL_API)) else True
    ...
    def teardown_config_session(self):
        ...
        if not _run('{0} teardownSession'.format(VYOS_SHELL_API)):
            logger.info('Cleaning up session environment variables')
    ...
    def session_changed(self):
        ...
        """
        if _run('{0} sessionChanged'.format(VYOS_SHELL_API)):
            return False
    ...
    def save(self):
        """
        Save applied modifications. Changes still persistent even after
        system   reboot.
        """
