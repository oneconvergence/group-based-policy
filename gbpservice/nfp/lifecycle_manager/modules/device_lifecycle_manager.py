...
def events_init(controller, config):
    ...
    notification_lifecycle_mgr_obj = NotificationHandler(controller, config)
    evs = [
        ...
        Event(id='NF_DEVICE_NOTIFICATION',
              handler=notification_lifecycle_mgr_obj),
    ]
...
class NotificationHandler(core_pt.PollEventDesc):
    ...
    def handle_event(self, ev):
        if ev.id == 'NF_DEVICE_NOTIFICATION':
            data = ev.data
            self.network_function_device_notification(data)
    ...
    def network_function_device_notification(self, notification_data):
        ...
        for response in responses:
            ...
            if resource == 'healthmonitor':
                LOG.info(
                    _("healthmonitor not implemented yet, so ignoring for now."))
                return None
...
class DeviceLifeCycleHandler(object):
    ...
    def _get_device_data(self, nfd_request):
        ...
        if network_function:
            ...
            device_data['service_chain_id'] = network_function[
                'service_chain_id']
...
class DLCMConfiguratorRpcApi(object):
    ...
    def create_network_function_device_config(self, device_data,
        ...
        LOG.info(
            _("create_network_function_device_config - config_params = %s" % config_params))
        data = {'request_data': config_params, 'context': self.context}
        ev = self._controller.new_event(id='NF_DEVICE_CONFIG_CREATE',
                                        data=data,
                                        key='NF_DEVICE_CONFIG_CREATE')
        self._controller.post_event(ev)
    ...
    def delete_network_function_device_config(self, device_data,
        ...
        LOG.info(
            _("delete_network_function_device_config - config_params = %s" % config_params))
        data = {'request_data': config_params, 'context': self.context}
        ev = self._controller.new_event(id='NF_DEVICE_CONFIG_DELETE',
                                        data=data,
                                        key='NF_DEVICE_CONFIG_DELETE')
        self._controller.post_event(ev)
