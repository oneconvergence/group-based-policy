...
class ServiceLifeCycleHandler(object):
    ...
    def handle_event(self, event):
        try:
            ...
            LOG.exception(_("Unhandled exception in handle event for event: %(event_id)s"), {
                          'event_id': event.id})

    def handle_poll_event(self, event):
        LOG.info(_("Service LCM handle_poll_event, event ID %(id)s"),
                 {'id': event.id})
        try:
            event_handler = self.event_method_mapping(event.id)
            event_handler(event)
    ...
    def handle_device_created(self, event):
        ...
        LOG.debug("handle_device_created heat_stack_id: %s" %
    ...
    def check_for_user_config_deleted(self, event):
        ...
        try:
            ...
            LOG.error(
                _("Checking is_config_delete_complete failed. Error: %(err)s"), {'err': err})
        if config_status == "ERROR":
            event_data = {
    ...
    # TODO: When Device LCM deletes Device DB, the Foreign key NSI will be
    # nulled
    ...
    def get_network_function(self, context, network_function_id):
        try:
            network_function = self.db_handler.get_network_function(
                self.db_session, network_function_id)
            LOG.info(_("In get_network_function, returning: %(network_function)s"), {
