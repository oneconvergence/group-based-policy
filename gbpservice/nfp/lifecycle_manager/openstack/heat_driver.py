...
class HeatDriver():
    ...
    def is_config_complete(self, stack_id, tenant_id):
        ...
        heatclient = self._get_heat_client(
            resource_owner_tenant_id, tenant_id=tenant_id)

        try:
            stack = heatclient.get(stack_id)
            LOG.info(_("Stack %(stack)s status is %(status)s"), {
                     'stack': stack_id, 'status': stack.stack_status})
            if stack.stack_status == 'DELETE_FAILED':
